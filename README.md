# 🐺 Big Bad Wolf — Slot Machine

A browser-based, western-themed slot machine inspired by the "huff-and-puff"
genre: a 5×3, **243-ways** game with a Hard-Hat Free-Spins bonus where the Big
Bad Wolf blows down straw / stick / brick houses for prizes, plus a Mansion
Jackpot. Pure vanilla HTML/CSS/JS — no framework, no runtime dependencies.

---

## Quick start

**Just play it:** open `index.html` in a browser (double-click works — it runs
straight off the `file://` protocol).

**Local web server** (needed for the in-game High-Noon easter egg's autoplay and
for a deploy-like environment):

```bash
node tools/serve.js        # serves the folder at http://localhost:8123
```

**After changing any source file under `src/`, rebuild the bundle:**

```bash
node tools/build.js        # bundles src/ → app.js
```

The browser only ever loads the single generated **`app.js`** — see *How the
build works* below.

---

## Project structure

```
.
├── index.html             The page. Loads app.js + styles.css.
├── app.js                 GENERATED bundle of src/ — the only script the page loads.
├── styles.css             All styling.
├── vercel.json            Deploy config: security headers for the published site.
├── README.md              You are here.
│
├── src/                   ── ALL SOURCE (ES modules, bundled into app.js) ──
│   ├── main.js              Entry point: imports everything, runs init().
│   │
│   ├── math/                THE MATH (start here to understand payouts)
│   │   ├── par-sheet.js       The par sheet: symbols, paytable, reel strips,
│   │   │                      bonus config, RTP models, constants. One source of truth.
│   │   └── mathcore.js        Pure math: 243-ways evaluation + bonus award rolls.
│   │                          No DOM — imported by both the game AND tools/sim.js.
│   │
│   ├── core/                state.js (shared runtime state) · utils.js (helpers)
│   ├── audio/               sound.js (sfx + music) · narrator.js · phrases.js (wolf script)
│   ├── render/              reels.js (reel animation) · particles.js · readouts.js (HUD)
│   ├── game/                base-game.js · bonus.js · buy-bonus.js
│   ├── panels/              The drawer pop-ups:
│   │                          options-drawer · deposit · rtp-picker · game-size ·
│   │                          simulator (📊 SIM) · math-breakdown (🧮 MATH) ·
│   │                          size-manifest (GENERATED by build.js)
│   ├── scenes/              Cinematic / ambient: intro · reveal · day-night ·
│   │                          high-noon (easter egg) · idle-poster
│   └── system/              dev-mode.js (gates admin tools — currently forced ON for everyone)
│
├── tools/                 ── DEV SCRIPTS (Node, never shipped to the browser) ──
│   ├── build.js             Bundle src/ → app.js (+ regenerate the size manifest).
│   ├── sim.js               Headless Monte-Carlo RTP verifier.
│   ├── serve.js             Tiny static file server for local testing.
│   ├── voice.js             Generate the narrator clips via ElevenLabs.
│   ├── music.js             Generate the base/bonus music via ElevenLabs.
│   ├── sfx.js               Generate the sound effects via ElevenLabs.
│   ├── alpha-mp4.js         Make HEVC-alpha .mp4 twins of transparent clips (Safari support).
│   └── process_images.py    Helper: knock out white image backgrounds.
│
├── docs/                  ── DELIVERABLE DOCS (math + QA + reference, all PDF) ──
│   ├── Math_Model_97%_RTP.xlsx     full par sheet as a workbook
│   ├── Math_Overview_97%_RTP.pdf   verification summary + overview
│   ├── Math_Model_85%_RTP.xlsx     the 85% "Lean" model
│   ├── Math_Overview_85%_RTP.pdf
│   ├── QA_Report.pdf · Would_This_Game_Pass_GLI_Certification.pdf
│   ├── Wolf_Voiceover_Script.pdf · Adaptive_Soundtrack_Guide.pdf
│   └── Keyboard_Shortcuts.pdf · Math_Flow_Chart_Claude.pdf · Math_Flow_Chart_Gemini.pdf
│
└── assets/                Media (images, video, audio) — version-controlled via Git LFS (see .gitattributes).
    ├── audio/ {music, sfx, narrator}
    ├── webm/              transparent + cutscene videos
    └── *.webp             reel symbols + bonus art (WebP, web-optimized)
```

---

## How the build works

Browsers block native ES-module `import` over the `file://` protocol, so the
clean module source in `src/` is bundled into one plain classic script, `app.js`,
by `tools/build.js`. Each module is wrapped in its own scope with a tiny
CommonJS-style `require()`, and imports are resolved by the module's **basename**
(so `import '../math/par-sheet.js'` becomes `require("par-sheet")`). That means
you can freely move files between folders without breaking the build — the only
rule is **basenames must stay unique** across `src/` (the build throws if not).

`index.html` loads only `app.js`, so the entire `src/` tree and `tools/` are
purely for development — never required at runtime by the browser.

---

## The math (and how to verify it)

Everything that decides money lives in two files, both heavily commented:

- **`src/math/par-sheet.js`** — the par sheet. Symbol paytable (per-way
  multipliers), per-reel symbol counts, the bonus award table, and named
  constants. Change a number here and the whole game changes.
- **`src/math/mathcore.js`** — the pure logic: how 243-ways pays are computed and
  how bonus awards roll. No DOM, no randomness hidden anywhere else.

Full math documentation for each RTP model lives in **`docs/`** (an Excel
workbook + a Word verification overview per model), and the same numbers power
the in-game **🧮 MATH** and **📊 SIM** panels.

**Two selectable RTP models** ship in the game (chosen in the RTP picker):

| Model | Total RTP | Base | Bonus | Bonus trigger |
|---|--:|--:|--:|--:|
| **Standard** | ~97% | ~49% | ~48% | ~1 in 180 |
| **Lean** | ~85% | ~43% | ~42% | ~1 in 180 |

Both share the same reels, hit frequency (~26%) and high-volatility feel —
"Lean" simply scales every win magnitude down for higher-margin placements.

**Verify the return-to-player with a Monte-Carlo run:**

```bash
node tools/sim.js 50000000                       # Standard (~97%)
BBW_RTP_MODEL=lean node tools/sim.js 50000000    # Lean (~85%)
```

> The bonus is high-variance (rare house/mansion jackpots), so a single run can
> wobble ±~0.5%; the figures above are the converged values over large samples.

---

## Developer / admin tools

The **"DEV MODE"** tools — RTP picker, GAME SIZE, Add-Credit, and the
📊 SIM / 🧮 MATH debug panel — are grouped under a "DEV MODE" header in the
options drawer. They are **currently forced ON for everyone, on every build
(including web)** — no `?dev=1` needed. The backtick (`` ` ``) key toggles the
SIM/MATH panel.

> **Restoring the gate:** dev mode is forced on by a one-line override in
> `src/system/dev-mode.js` (`export const DEV_MODE = true;`). To go back to the
> hidden-by-default behavior — enable via `?dev=1`, remembered in localStorage,
> `?dev=0` to disable — change that line back to `export const DEV_MODE =
> compute();` and rerun `node tools/build.js`. The original gating logic in
> `compute()` is kept intact for exactly this.

(Buy Bonus is a normal player feature, so it stays visible in regular mode.)

---

## The wolf's voice (contextual VO)

The Big Bad Wolf reacts to what's actually happening. He plays pre-rendered
ElevenLabs clips (no live TTS), but **which** line and **when** is chosen from
real game state.

- **`src/audio/phrases.js`** — every line, grouped into ~50 pools (one source of
  truth, shared with the generator). Symbol callouts map to what's *visible*:
  shot glass = `toolbox`, horseshoe = `pig-suit`/`pig-contractor`, hard hats =
  the bonus symbols, Wolf Wild = `wild`.
- **`src/audio/narrator.js`** — the `WolfVO` manager. It scans the visible grid,
  tracks win tier / win+loss streaks / bonus vs base mode / idle time, and picks
  with per-category cooldowns, spin-gaps, no-repeat, and priority (big moments
  interrupt idle; he never overlaps himself). Idle lines reference the last
  outcome ("admirin' the win?"). Reacts to: expanding reels, anticipation,
  symbol callouts, 2/3/4+ streaks, gentle cold streaks, bonus enter/win/end,
  and menu return. Responsible-play safe — no "you're due", no pressure.

**Add lines:** drop them into the right array in `phrases.js`, then generate just
the new ones:

```bash
node tools/voice.js sync          # incremental — only new/changed clips (recommended)
```

The voice is fixed (Callum-husky, recorded in `tools/voice.js`); `sync` keeps a
`.vo-cache.json` text-hash so existing clips are never re-billed. `node
tools/voice.js all` wipes and regenerates everything.

**Debug / QA:** in the console set `window.WOLF_VO_DEBUG = true` to log every
decision (event, candidates, pick, why others were filtered). `window.__wolfVO`
is the live manager — e.g. `__wolfVO.onWin(100, 1)`. The friendly "other
huff-and-puff act" roast pool is **off by default**; enable with
`__wolfVO.ENABLE_ROASTS = true`.

---

## Deploying

Run `./deploy.sh` to build and publish to Vercel (first time: `vercel login` then
`vercel link`). The script bundles a clean, secrets-free folder — only the files
the browser needs (index.html, app.js, styles.css, vercel.json, assets/) — so
nothing local-only is ever served. `vercel.json` adds security headers on top.
The media in `assets/` is version-controlled via Git LFS (see .gitattributes),
so collaborators get it on clone and the deploy bundles it from there.

There's also a hidden surprise at exactly 12:00 PM local time. 🤠
